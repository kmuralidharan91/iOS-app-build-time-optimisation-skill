---
name: ios-build-fix
description: Apply a single approved finding from ios-build-diagnose to an Xcode/Tuist/Bazel project on a throwaway branch, re-measure with ios-build-measure, report wall-clock delta, and refuse to claim success when the delta is null, regressive, or within variance noise. Single-fix-at-a-time; atomic git-aware (creates a branch named ios-build-fix/<rule-slug>-<timestamp>); predicted-vs-actual logged for every run. Auto-applicable v1 surface is F1 (random-sleep), F3 (sandbox+fuse via xcconfig), F4 (compilation-cache-disabled), F9 (eager-linking-disabled, designed null-delta refusal-path test) — all Xcode-side (Tuist generates xcconfig, so these apply via the same path post-`tuist generate`); F5/F6/F7 emit a manual recipe and refuse to mutate the tree without --allow-manual. On Bazel projects, F1 / F3 dispatch to informational stubs with manual BUILD.bazel recipes in the preview (outcome=refused-null); buildozer-backed auto-apply lands in v1.4. Use when the user wants a fix applied + verified end-to-end, not just predicted; refuse politely otherwise.
