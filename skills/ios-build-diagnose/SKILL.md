...
1. **Detect** the build system at `--project-path`. v1.3 ships diagnose end-to-end for **Xcode, Bazel, and Tuist** projects. Tuist routes through `tuist_adapter` (which runs `tuist generate` then delegates to `xcode_adapter`); Bazel routes through `bazel_adapter` (`bazel query 'kind(genrule, //...)'` for ScriptPhase, `bazel info` + `bazel cquery --output=jsonproto` for build settings, Package.resolved walk for the SPM graph). Xcode-only build settings (F4 / F9 / sandboxing / fuse) are auto-suppressed on Bazel projects so no spurious findings surface. The detector is shared with `ios-build-measure` (`scripts/adapters/__init__.py`).
