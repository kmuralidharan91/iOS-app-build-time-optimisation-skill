...
**Required adapter API** (every adapter implements these signatures, draft):
...
def show_build_settings(project_path: pathlib.Path, scheme: str | None,
                        configuration: str, platform: Literal["ios"]) -> dict: ...
def script_phases(project_path: pathlib.Path) -> list[ScriptPhase]: ...
def package_graph(project_path: pathlib.Path) -> PackageGraph: ...
def apply_fix(project_path: pathlib.Path, finding_id: str,
              patch: ApprovedPatch, branch_name: str) -> FixResult: ...
```

The Xcode adapter is the v1-must-ship target. Tuist + Bazel adapters ship measurement parts in v1; full diagnose for non-Xcode systems may slip to v2 if effectiveness gates can't be hit.
