...
2. **Run** `--repeats` builds per requested kind. For Xcode targets, clean runs invoke `xcodebuild clean` then `xcodebuild build -showBuildTimingSummary -resultBundlePath …`. For Bazel targets (v1.1+), clean runs invoke `bazelisk clean --expunge` then `bazelisk build --config=<configuration> --profile=<output_dir>/build-…-profile.json <scheme>`; the `--scheme` flag accepts a Bazel target label (e.g. `//App:SmokeLib`) and `--configuration` maps to `bazelisk --config=<name>`. Incremental runs `touch` `--touch-file` then build without cleaning. Each run is wall-clock-timed with `time.monotonic`.
